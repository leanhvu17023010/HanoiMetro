...
public class ChatService {
    ...
    public ChatMessageResponse sendMessage(SendMessageRequest request) {
        ...
        if (sender.getRole() != null && sender.getRole().getName().equals("CUSTOMER")) {
            if (receiver.getRole() == null || !receiver.getRole().getName().equals("CUSTOMER_SUPPORT")) {
        ...
        try {
            notificationService.sendToUsers(
                "Tin nhắn mới",
                String.format("Bạn có tin nhắn mới từ %s", sender.getFullName() != null ? sender.getFullName() : sender.getEmail()),
                "CHAT",
    ...
    public List<ChatConversationResponse> getConversations() {
        ...
        List<User> partnersAsSender = chatMessageRepository.findChatPartnersAsSender(currentUser.getId());
        List<User> partnersAsReceiver = chatMessageRepository.findChatPartnersAsReceiver(currentUser.getId());
        ...
        return partners.stream().map(partner -> {
            ...
            Long unreadFromPartner = messages.stream()
    ...
    public void markAsRead(String partnerId) {
        ...
        List<ChatMessage> unreadMessages = chatMessageRepository.findConversationBetweenUsers(
                currentUser.getId(), partner.getId())
                .stream()
                .filter(m -> m.getReceiver().getId().equals(currentUser.getId()) && !m.getIsRead())
                .collect(Collectors.toList());
    ...
    private ChatMessageResponse toResponse(ChatMessage message) {
        return ChatMessageResponse.builder()
                ...
                .receiverName(message.getReceiver().getFullName() != null ? 
                    message.getReceiver().getFullName() : message.getReceiver().getEmail())
